...
#include "../spawnMenu.h"
#include "../miscMenu.h"
#include "object/spawnObjectSettingsMenu.h"
#include "manager/spawnEntityManager.h"

using namespace SpawnObjectMenuVars;

namespace SpawnObjectMenuVars {
	Object SpawnObject(String toSpawn, bool blip) {
		float forward = 2.f;
		...
		if (STREAMING::IsModelInCdimage(model)) {
			...
			object = OBJECT::CreateObject(model, GetLocalPlayer().m_coordinates.x + xVect, GetLocalPlayer().m_coordinates.y + yVect, GetLocalPlayer().m_coordinates.z, 1, 1, 0);
			if (blip) { auto blip = HUD::AddBlipForEntity(object); HUD::SetBlipSprite(blip, 351); HUD::SetBlipColour(blip, 4); }

			if (SpawnObjectSettingsMenuVars::vars.particles) {
				Vector3 coords = ENTITY::GetEntityCoords(object, 1);
		...
		if (ENTITY::DoesEntityExist(object)) {
			for (int i = 0; i < 500; i++) {
				if (!SpawnEntityManagerMenuVars::Spawned[i].m_taken) {
					...
					if (SpawnObjectSettingsMenuVars::vars.information) {
	...
	void SpawnObjectPlayer(DWORD model, int playerID) {
		SPlayer& player = GetPlayer(playerID);
		if (STREAMING::IsModelInCdimage(model)) {
			GetEntityControl()->SimpleRequestModel(model);
			OBJECT::CreateObject(model, player.m_coordinates.x, player.m_coordinates.y, player.m_coordinates.z, 1, 1, 0);
		}
	}

	void SpawnObject(String model) {
		SpawnObject(model, SpawnObjectSettingsMenuVars::vars.blip);
	}
...
void SpawnObjectMenu::Init() {
	setParentSubmenu<SpawnMenu>();
	setName("Spawn object", true, false);

	addOption(SubmenuOption("Spawner settings")
		.addTooltip("Spawner settings")
		.addSubmenu<SpawnObjectSettingsMenu>().addTranslation());

	addOption(SubmenuOption("Manage spawned objects")
		.addSubmenu<SpawnEntityManagerMenu>()
		.addFunction([] { SpawnEntityManagerMenu::GetInstance()->setParentSubmenu<SpawnObjectMenu>(); })
		.addTooltip("Manage the objects you've spawned").addTranslation());

	addOption(ButtonOption("Input object name")
		.addFunction([] {
		GetMenuInput()->AddToQueue([] {
			string temp;
			if (GetMenuInput()->Text("Object name", temp, 2, 40)) {
				if (!SpawnMenuVars::vars.CreatorMode) {
					SpawnObject(temp.c_str(), SpawnObjectSettingsMenuVars::vars.blip);
				}
				else {
					SpawnMenuVars::vars.SelectedCreatorObject = temp.c_str();
					SpawnMenuVars::vars.IsObjectAPed = false;
				}
			}});
		})
		.addHotkey().addTranslation()
		.addTooltip("Input object name").addTranslation());

	addOption(BreakOption("Objects").addTranslation());

	for (int i = 0; i < 9465; i++) {
		addOption(ButtonOption(m_objectmodels[i])
			.addFunction([=] {
			if (SpawnMenuVars::vars.CreatorMode) {
				SpawnMenuVars::vars.SelectedCreatorObject = m_objectmodels[i];
				SpawnMenuVars::vars.IsObjectAPed = false;
			}
			else {
				GetFiberPool()->DoJob([=] { SpawnObject(m_objectmodels[i]); });
			}
		})
			.addTooltip("Spawn this object"));
	}
}

/*Update once when submenu is opened*/
void SpawnObjectMenu::UpdateOnce() {}

/*Update while submenu is active*/
void SpawnObjectMenu::Update() {}

/*Background update*/
void SpawnObjectMenu::FeatureUpdate() {
	
}

/*Singleton*/
SpawnObjectMenu* _instance;
SpawnObjectMenu* SpawnObjectMenu::GetInstance() {
	if (_instance == nullptr) {
		_instance = new SpawnObjectMenu();
		_instance->Init();
		GetSubmenuHandler()->addSubmenu(_instance);
	}
	return _instance;
}
SpawnObjectMenu::~SpawnObjectMenu() { delete _instance; }